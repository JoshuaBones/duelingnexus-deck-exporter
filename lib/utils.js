...
function serverDecksToDeckObject(decksFromServer) {
    return decksFromServer.map(deck => {
        const main = deck.main_deck === '' ? [] : deck.main_deck.split(',');
        const extra = deck.extra_deck === '' ? [] : deck.extra_deck.split(',');
        const side = deck.side_deck === '' ? [] : deck.side_deck.split(',');
...
async function downloadDecks(id=null, deckPreProcesseses=null) {
    ...
    decks.forEach(deck => {
        ...
        zip.file(`${deck.name}.ydk`, buildDeckText(deck.main, deck.extra, deck.side));
    });
...
function replaceAllDeckErratas(decks) {
    ...
    decks.forEach((deck, deckI) => {//each deck
        errataIds.forEach((errataId, idx) => {//each errata id
        ...
        for (let section in deck) {//property
            if (Array.isArray(deck[section])) {//main/extra/side
                ...
                cards.forEach((cardId, index) => {//main, extra or side's cards
                    ...
                    const cardData = data.cards.find(c => c.id === intCardId);//^card data from json
                    if('als' in cardData && cardData.als != 0) {//als references the original card's id, but does not exist if it's the original
